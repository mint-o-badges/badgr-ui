	...
	template: `
		...
		>
			<hlm-trow
				[ngClass]="
					requests.length > 0
						? 'tw-bg-green hover:!tw-bg-green tw-text-purple'
						: 'tw-bg-darkgrey tw-text-white'
				"
				class="tw-bg-darkgrey tw-flex-wrap hover:!tw-bg-darkgrey"
			>
				<hlm-th
					[ngClass]="requests.length > 0 ? '!tw-text-purple' : '!tw-text-white'"
					class="tw-w-40 tw-font-medium"
					>ID</hlm-th
				>
				<hlm-th
					[ngClass]="requests.length > 0 ? '!tw-text-purple' : '!tw-text-white'"
					class="tw-justify-center tw-font-medium xl:tw-pr-12 !tw-flex-1"
					>Angefragt am
				</hlm-th>
				<hlm-th
					[ngClass]="requests.length > 0 ? '!tw-text-purple' : '!tw-text-white'"
					class="tw-justify-end tw-font-medium xl:tw-w-40 tw-w-0 !tw-p-0"
				></hlm-th>
			</hlm-trow>
			<hlm-trow
			...
			>
				...
				<hlm-th class="!tw-flex-1 tw-justify-center !tw-text-purple"
					><p class="u-text">{{ request.requestedOn | date: 'dd.MM.yyyy' }}</p></hlm-th
				>
				...
				<hlm-th class="tw-justify-center sm:tw-justify-end sm:tw-w-48 tw-w-full !tw-text-oebblack">
					<oeb-button
						...
						loading-message="Badge vergeben..."
					></oeb-button>
				</hlm-th>
			</hlm-trow>
		</hlm-table>
