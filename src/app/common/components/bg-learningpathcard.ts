	...
	template: `
		<a [routerLink]="['/public/learningpaths/', slug]">
			<div class="tw-flex tw-flex-col tw-justify-between tw-h-full">
				<div
					class="tw-bg-[var(--color-lightgray)] tw-w-full tw-relative md:tw-h-[175px] tw-h-[100px] tw-items-center tw-flex tw-justify-center tw-p-2 tw-rounded-[3px]"
				>
					<div *ngIf="!completed" class="tw-absolute tw-top-[10px] tw-right-[10px]">
						<img
							src="/assets/oeb/images/learningPath/learningPathIcon.svg"
							class="md:tw-w-[30px] tw-w-[20px]"
							alt="LearningPath"
						/>
					</div>
					<div
					...
					>
						...
						>
							<hlm-icon
								class="tw-text-purple tw-box-border md:tw-w-[26px] tw-w-[16px] md:tw-h-[26px] tw-h-[16px]"
								name="lucideCheck"
							/>
						</div>
					</div>
				...
				<div class="tw-flex tw-flex-col tw-flex-wrap tw-py-2 tw-text-oebblack tw-mt-2 tw-gap-1">
					<span class="tw-font-semibold tw-text-xs md:tw-text-[22px] md:tw-leading-[26px]">{{ name }}</span>
					<a class="md:tw-text-[18px] md:tw-leading-[23.4px] tw-text-[10px] tw-leading-[13px]">{{
						issuerTitle
					}}</a>
					<div class="tw-items-center">
						...
						<ng-template #progressBar>
							<div
								*ngIf="progress === 0 || progress"
								class="tw-mb-4 tw-w-full tw-mt-2 md:tw-mt-6 tw-flex tw-justify-center tw-items-center"
							>
								<oeb-progress
									class="tw-w-full tw-h-5 md:tw-h-7 tw-relative tw-inline-flex tw-overflow-hidden tw-rounded-3xl tw-bg-white tw-items-center"
									[value]="progressValue"
						...
						<ng-template #progressTemplate>
							<div class="tw-absolute tw-w-full tw-text-left">
								<span class="tw-ml-2 md:tw-text-sm tw-text-[8px] tw-text-purple"
									>Lernpfad <span *ngIf="!completed">{{ progressValue }}%</span> abgeschlossen</span
								>
							</div>
						</ng-template>
						...
						<oeb-button
							*ngIf="isProgress && progress / studyLoad === 1 && !completed && !requested"
							(click)="requestLearningPath()"
							[fontSize15]="true"
							[text]="'Lernpfad-Badge abholen'"
							width="full_width"
						>
						</oeb-button>
					</div>
					<div
						class="tw-flex tw-flex-row tw-gap-4 tw-text-[#6B7280] md:tw-text-sm tw-text-[8px] tw-mt-2 md:tw-mt-6 tw-items-end"
					>
